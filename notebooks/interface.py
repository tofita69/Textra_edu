...
# Using my fine-tuned Bart model
...
bart_pipeline = pipeline(
    ...
    device=bart_model.device,  # Ensure the pipeline is on the same device as the model
    max_new_tokens=512,
)
...
# Streamlit App Title
st.title("Textra-edu")
st.markdown("Using Fine-tuned BART Model for Summarization and Simple Question Answering")
...
if app_mode == "Summarizer":
    ...
    def generate_summary(input_text):
        try:
            ...
            input_ids = inputs["input_ids"].to(bart_model.device)
            attention_mask = inputs["attention_mask"].to(bart_model.device)
